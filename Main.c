...
void callback_file (GtkMenuItem *menuitem, gpointer user_data)
{
    ...
    data->currentWindow = GTK_WIDGET (gtk_builder_get_object (data->builder, 
                                                            "FileSelector"));
...
void callback_image(GtkButton *open, gpointer user_data)
{
    ...
    const gchar *filename = 
        gtk_file_chooser_get_filename(GTK_FILE_CHOOSER(data->currentWindow));
    ...
    GtkButton *analyze = GTK_BUTTON(gtk_builder_get_object(data->builder, 
                                                                    "Analyze"));
...
void callback_OCR(GtkButton *analyze, gpointer user_data)
{
    ...
    GtkTextBuffer *buff = GTK_TEXT_BUFFER(gtk_builder_get_object(data->builder,
                                                                     "Buffer"));
    ...
    char *resultChar = calloc(200, sizeof(char));
    ssize_t read;
    char *line = NULL;
...
void callback_about(GtkMenuItem *menuitem, gpointer user_data)
{
    ...
    GtkWidget *about = GTK_WIDGET(gtk_builder_get_object (data->builder, 
                                                                    "About"));
...
void callback_save(GtkButton *save, gpointer user_data)
{
    ...
    GtkEntry *entry = GTK_ENTRY(gtk_builder_get_object (data->builder, 
                                                                "SaveEntry"));

    GtkFileChooser *file = GTK_FILE_CHOOSER(gtk_builder_get_object 
                                                (data->builder, "FileSaver"));
